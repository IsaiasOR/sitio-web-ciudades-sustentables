export const temasInvestigacion = {
    topics: [
        "Responsabilidad Social.",
        "Sustentabilidad. Desarrollo Sostenible.",
        "Gobiernos Locales. Organizaciones del ámbito público.",
        "Indicadores de medición de la RS y el DS.",
        "Gestión de la información.",
        "Transparencia y rendición de cuentas.",
        "Grupos de interés. Stakeholders.",
        "Agenda 2030 y ODS.",
        "Ciudades sostenibles/sustentables.",
    ]
}

export const metodologia = {
    methodology: "Metodología - Trabajo de campo: nuestro universo esta conformado por los gobiernos locales del corredor del Río Uruguay en la provincia de Entre Ríos."
}